...
Functionality that has been move to plugins for ths release:
...
Changes with direct rule impact (FIXME: Changes after RC1):
...
Changes with direct rule impact (FIXME: Changes before RC1; to be joined with section above):
 FIXME: Section with entries containing rule id
 ...
 * Fix chained condition for correlation rules 980120 and 980150 (Simon Studer) [#1898]
 ...
 FIXME: Section with entries lacking rule id
 ...
 * fix: avoid FP in CalDAV Content-Type (Vandan Rohatgi) [#2505]
 * fix: avoid FP in MySQL data leakage rule (Jozef Sudolský) [#2490]
 * fix: avoid FP in SQL function names by splitting between PL1/PL2 (Jozef Sudolský) [#2480]
 * fix: avoid FP in 'having' SQLi (Felipe Zipitría) [#2248]
 * fix: avoid FP in JWT tokens (Andrea Menin) [#2460]
 * fix: avoid FP on 'time' and 'ping' keywords (Walter Hop) [#2457]
 * fix: avoid FP in 'application/*+json' Content-Type (Mirko Dziadzka, Walter Hop) [#2455]
 * fix: avoid FP in Sec-CH-UA header (Franziska Bühler) [#2361]
 * fix: avoid FP in PostgreSQL error messages (Jozef Sudolský, Franziska Bühler) [#1870, #2313]
 * fix: avoid FP in Sybase error message (Jozef Sudolský) [#2307]
 * fix: avoid FP in base64 content (Jozef Sudolský) [#2226]
 * fix: avoid FP in XMLNLS (Walter Hop) [#2192]
 * fix: avoid FP in SQLi by adding word boundary checks (Jozef Sudolský) [#2078]
 * fix: avoid FP in SQLi with keyword 'union' (Franziska Bühler) [#2058]
 * fix: avoid FP for YAM package manager (Jozef Sudolský) [#2022]
 * fix: avoid FP for '..' without slashes (Tetrik, Walter Hop) [#2016]
 * fix: avoid FP in Google OAuth2 (Jozef Sudolský, Christian Folini) [#1958]
 * fix: avoid FP on .axd in restricted extensions, these are public (Jozef Sudolský) [#1925]
 * fix: avoid FP on 'version' string in Content-Type header (Jozef Sudolský) [#1901]
 * fix: avoid FP in Content-Type with '#' character (Jozef Sudolský) [#1856]
 * update: improve sampling mode efficiency (Paul Beckett) [#2094]
 * Move rules to the earliest phase possible based on their inputs (Ervin Hegedus) [#1941]
 ...
 * Added missing quotes from the end of actions list (Ervin Hegedus) [#2184]
